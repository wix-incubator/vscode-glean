...
function getRenderFunctionBody(statelessComponentBody) {
  if (t.isBlockStatement(statelessComponentBody)) {
    const returnStatement = <t.ReturnStatement>(
      statelessComponentBody.body.find(
        bodyContent => bodyContent.type === "ReturnStatement"
      )
    );
    const returnStatementContent = returnStatement.argument;
    if (!t.isParenthesizedExpression(returnStatementContent)) {
      const parenthesizedReturnStatement = t.parenthesizedExpression(
        returnStatementContent
      );
    ...
    const body = t.isParenthesizedExpression(statelessComponentBody)
      ? statelessComponentBody
      : t.parenthesizedExpression(statelessComponentBody);
...
const getParamTypeAnnotation = param => {
  return param && param.typeAnnotation
    ? t.tsTypeParameterInstantiation([param.typeAnnotation.typeAnnotation])
    : null;
};

const getDeclarationTypeAnnotation = declaration => {
  return declaration && declaration.typeAnnotation
    ? t.tsTypeParameterInstantiation([
        declaration.typeAnnotation.typeAnnotation.typeParameters.params[0]
      ])
    : null;
};
    ...
    Function(path) {
      if (path.node.params.length) {
        if (path.node.params[0].type === "ObjectPattern") {
          ...
          path.scope.bindings[path.node.params[0].name].referencePaths.forEach(
            refPath => {
      ...
      const typeAnnotation =
        getParamTypeAnnotation(path.node.params[0]) ||
        getDeclarationTypeAnnotation((<any>path).container.id);
      const render = t.classMethod(
        "method",
        t.identifier("render"),
        [],
        getRenderFunctionBody(path.node.body)
      );
      const superCall = t.expressionStatement(
        t.callExpression((<any>t).super(), [t.identifier("props")])
      );
      const ctor = t.classMethod(
        "constructor",
        t.identifier("constructor"),
        [t.identifier("props")],
        t.blockStatement([superCall])
      );
      const classDefinition = t.classDeclaration(
        name,
        t.identifier("Component"),
        t.classBody([ctor, render])
      );
      classDefinition.superTypeParameters = typeAnnotation;

      if (
        t.isArrowFunctionExpression(path) &&
        !t.isExportDefaultDeclaration((<any>path).parentPath)
      ) {
