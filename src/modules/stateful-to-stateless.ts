...
export function statefulToStateless(component) {
  ...
  const arrowFunction = ({
    ...
    identifier.typeAnnotation = propType
      ? t.tsTypeAnnotation(
          t.tsTypeReference(
            t.identifier("SFC"),
            t.tsTypeParameterInstantiation([propType])
          )
        )
      : null;
  ...
  const appendFunctionBodyToStatelessComponent = (name, body) => {
    ...
    if (t.isExportDefaultDeclaration(statelessComponentPath)) {
      statelessComponentBodyBlock =
        statelessComponentPath["node"].declaration.body.body;
    } else if (t.isExportNamedDeclaration(statelessComponentPath)) {
      statelessComponentBodyBlock =
        statelessComponentPath["node"].declaration.declarations[0].init.body
          .body;
    } else {
      statelessComponentBodyBlock =
        statelessComponentPath.node.declarations[0].init.body.body;
    }
      ...
      const statelessComponent = arrowFunction({
        ...
        propType:
          ...
          path.node.superTypeParameters.params.length
            ? path.node.superTypeParameters.params[0]
            : null,
